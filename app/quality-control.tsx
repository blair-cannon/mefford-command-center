...
import { inspectionResult } from "../lib/quality-control";
import { summaryDrilldownProps } from "./summary-drilldown";
import { CloseButton } from "./close-button";
import { FieldPhotoCapture } from "./field-capture";
...
export function QualityControlWorkspace({ project, actor }: { project: { number: string; name: string; projectManager: string; superintendent: string; substantialDate: string; timeZone: string }; actor: Actor }) {
  ...
  const uploadedPhotos = useRef(new Map<File, number>());
  ...
  async function load() {
    ...
    try {
      ...
      setData(result);
      setSelectedInspectionId((current) => current && result.inspections.some((item) => item.id === current) ? current : "");
  ...
  async function uploadPhotos(files: File[], category: string, revision: string) {
    const ids: number[] = [];
    setPhotoProgress({ uploaded: 0, processed: 0, total: files.length });
    for (const file of files) {
      const cached = uploadedPhotos.current.get(file);
      ...
      ids.push(result.file.id); uploadedPhotos.current.set(file, result.file.id);
      setPhotoProgress({ uploaded: ids.length, processed: ids.length, total: files.length });
    }
    return ids;
  }
  ...
  async function createItem() {
    ...
    if (itemDraft.requiresDesignerAcceptance && !itemDraft.designerVendorId) { setNotice("Select The Accepting Designer."); return; }
    setSaving(true); setNotice("");
    try {
    ...
    } catch (error) { setNotice(error instanceof Error ? error.message : "Photos Could Not Be Saved. Your Entries Are Still Here."); } finally { setSaving(false); }
  }
  ...
  function selectInspection(item: QualityRecord) {
    setSelectedInspectionId(item.id); setDetailOpen(true); setNotice(""); setPhotoProgress(null); uploadedPhotos.current.clear();
    setResponses([]);
    setInspectionDraft({ exactLocation: String(item.data.exactLocation || ""), responsibleTrade: String(item.data.responsibleTrade || ""), responsibleVendorId: String(item.data.responsibleVendorId || ""), generalNotes: "", requiresDesignerAcceptance: false, designerVendorId: "" });
    setInspectionPhotos([]);
  ...
  async function completeInspection() {
    if (!selectedInspection || !selectedTemplate) return;
    const checked = inspectionResult(selectedTemplate, responses);
    if (!checked.complete) { setNotice("Answer Every Required Inspection Question And Explain Each No Or N/A."); return; }
    if (checked.failed.length > 0 && (!inspectionPhotos.length || !inspectionDraft.exactLocation.trim() || !inspectionDraft.responsibleTrade.trim())) { setNotice("Failed Checks Require An Exact Location, Responsible Trade And Photos."); return; }
    setSaving(true); setNotice("");
    try {
  ...
  return <div className="quality-workspace" data-detail-open={detailOpen}>
    <header className="quality-hero"><div><p>{project.number} · {actor.name}</p><h1>Quality</h1></div><div><button className="quality-primary" onClick={openItem}>＋ New Quality Item</button></div></header>
    {notice ? <div className={/could not|required|only|locked|unavailable/i.test(notice) ? "quality-notice error" : "quality-notice"}>{notice}<CloseButton aria-label="Dismiss Message" onClick={() => setNotice("")} /></div> : null}
    <details className="field-compact-details"><summary>Quality Summary · {openInspections} Inspections Due · {openItems} Open Items</summary><section className="quality-metrics">
      ...
      <article {...summaryDrilldownProps({ title: "Field Verifications", rows: (data?.items || []).filter((item) => item.status === "Verification Requested").map((item) => ({ id: item.id, title: item.title, subtitle: `${String(item.data.exactLocation || "Location Required")} · Due ${formatDate(item.due)}`, status: item.status, onOpen: () => { setSelectedItemId(item.id); setDetailOpen(true); setView("Quality Items"); }, openLabel: "Open Quality Item →" })) })}><strong>{verificationCount}</strong><span>FIELD VERIFICATIONS</span><small>Waiting on Superintendent</small></article>
      <article {...summaryDrilldownProps({ title: "Open Quality Items", rows: (data?.items || []).filter((item) => item.status !== "Closed").map((item) => ({ id: item.id, title: item.title, subtitle: `${String(item.data.exactLocation || "Location Required")} · ${String(item.data.responsibleTrade || "Unassigned")}`, status: item.status, onOpen: () => { setSelectedItemId(item.id); setDetailOpen(true); setView("Quality Items"); }, openLabel: "Open Quality Item →" })) })}><strong>{openItems}</strong><span>OPEN QUALITY ITEMS</span><small>{data?.closeout.reachedSubstantialCompletion ? "In final punch tracking" : "Warn billing · block final"}</small></article>
    </section></details>
    <nav className="quality-tabs">{(["Pre-Work", "Quality Items", "Punch & Closeout", "Portal Access", "Template Library"] as View[]).map((item) => <button key={item} className={view === item ? "active" : ""} onClick={() => { setView(item); setDetailOpen(false); }}>{item}{item === "Pre-Work" && openInspections ? <b>{openInspections}</b> : item === "Quality Items" && openItems ? <b>{openItems}</b> : null}</button>)}</nav>

    {view === "Pre-Work" ? <div className="quality-register-layout" data-detail-open={detailOpen}>
      ...
      <main className="quality-detail"><button className="secondary-action field-mobile-back" onClick={() => setDetailOpen(false)}>← Back To List</button>{!selectedInspection ? <div className="quality-empty large"><b>Select A Pre-Work Request</b></div> : <>
        ...
        {["Superintendent Action Required", "Follow-Up Required"].includes(selectedInspection.status) && data?.permissions.canCompleteInspection && selectedTemplate ? <>
          <section className="inspection-context"><label>Exact Work Location<input value={inspectionDraft.exactLocation} onChange={(event) => setInspectionDraft((current) => ({ ...current, exactLocation: event.target.value }))} placeholder="Example: West wall · Grid A/4–A/8" /></label><label>Responsible Trade<input value={inspectionDraft.responsibleTrade} onChange={(event) => setInspectionDraft((current) => ({ ...current, responsibleTrade: event.target.value }))} /></label><label>Responsible Company<select value={inspectionDraft.responsibleVendorId} onChange={(event) => setInspectionDraft((current) => ({ ...current, responsibleVendorId: event.target.value, responsibleTrade: data?.vendors.find(vendor => vendor.id === event.target.value)?.trade || current.responsibleTrade }))}><option value="">Mefford / Not Yet Assigned</option>{data.vendors.map((vendor) => <option key={vendor.id} value={vendor.id}>{vendor.name}</option>)}</select></label></section>
          <section className="inspection-form"><header><div><h3>{selectedTemplate.title}</h3><p>{selectedTemplate.questions.length} controlled requirements from {selectedTemplate.sourceDocument}</p></div><span>{responses.filter((item) => item.answer).length}/{selectedTemplate.questions.length}</span></header>{groupedQuestions.map(({ group, questions }) => <div className="inspection-group" key={group}><h4>{group}</h4>{questions.map((question, index) => { const response = responses.find((item) => item.questionId === question.id); return <article key={question.id}><span className="question-number">{index + 1}</span><div><b>{question.label}</b>{question.input === "check" ? <div className="inspection-answers">{["Yes", "No", "N/A"].map((answer) => <button type="button" key={answer} aria-pressed={response?.answer === answer} aria-label={`${question.label}: ${answer}`} className={response?.answer === answer ? answer === "No" ? "active no" : "active" : ""} onClick={() => setAnswer(question, answer)}>{answer}</button>)}</div> : question.input === "choice" ? <select aria-label={question.label} value={response?.answer || ""} onChange={(event) => setAnswer(question, event.target.value)}><option value="">Select Required Answer</option>{question.choices?.map((answer) => <option key={answer}>{answer}</option>)}</select> : <input aria-label={question.label} value={response?.answer || ""} onChange={(event) => setAnswer(question, event.target.value)} />}{question.input === "check" && ["No", "N/A"].includes(response?.answer || "") ? <textarea aria-label={`${question.label}: Explanation`} rows={2} value={response?.note || ""} onChange={(event) => setResponseNote(question, event.target.value)} placeholder={response?.answer === "No" ? "Describe the deficiency or condition" : "Required N/A justification"} /> : null}</div></article>})}</div>)}</section>
          <section className="inspection-evidence"><label>General Inspection Notes<textarea rows={4} value={inspectionDraft.generalNotes} onChange={(event) => setInspectionDraft((current) => ({ ...current, generalNotes: event.target.value }))} /></label><FieldPhotoCapture files={inspectionPhotos} onChange={setInspectionPhotos} disabled={saving} label="Inspection / Before Photos" progress={photoProgress} /></section>
          {notice ? <p className="field-error" role="alert">{notice}</p> : null}<div className="quality-actions"><button onClick={() => setOverrideOpen(true)}>Document Qualified Override</button><button className="quality-primary" disabled={saving} onClick={() => void completeInspection()}>{saving ? "Recording…" : "Complete And Lock This Attempt"}</button></div>
        </> : <section className="quality-readonly"><h3>{["Superintendent Action Required", "Follow-Up Required"].includes(selectedInspection.status) ? "Awaiting Superintendent" : "Permanent Disposition"}</h3><p>{["Superintendent Action Required", "Follow-Up Required"].includes(selectedInspection.status) ? `Assigned To ${project.superintendent}` : selectedInspection.status === "Override Documented" ? `${String((selectedInspection.data.override as Record<string, unknown> | undefined)?.alternateChecklist || "Alternate full-scope checklist")} · ${String((selectedInspection.data.override as Record<string, unknown> | undefined)?.trade || "Trade")} · ${formatDate(String((selectedInspection.data.override as Record<string, unknown> | undefined)?.completionDate || ""))}` : `${String(selectedInspection.data.result || selectedInspection.status)} · Completed by ${String(selectedInspection.data.completedBy || selectedInspection.owner)}`}</p><AuditTrail record={selectedInspection} /></section>}
      </>}</main>
    </div> : null}

    {view === "Quality Items" ? <div className="quality-register-layout" data-detail-open={detailOpen}><aside className="quality-register"><header><h2>Deficiency Register</h2><span>{data?.items.length || 0} ITEMS</span></header>{!data?.items.length ? <div className="quality-empty"><b>No Quality Items Yet</b><p>PMs and Superintendents create formal items. Others submit for PM validation.</p></div> : data.items.map((item) => <button key={item.id} className={selectedItemId === item.id ? "active" : ""} onClick={() => { setSelectedItemId(item.id); setDetailOpen(true); }}><span><b>{item.id}</b><strong>{item.title}</strong><small>{String(item.data.exactLocation || "Location Required")} · {String(item.data.responsibleTrade || "Unassigned")}</small></span><i className={statusClass(item.status)}>{item.status}</i></button>)}</aside><main className="quality-detail"><button className="secondary-action field-mobile-back" onClick={() => setDetailOpen(false)}>← Back To List</button>{selectedItem ? <QualityItemDetail key={selectedItem.id} item={selectedItem} data={data!} saving={saving} onPost={post} /> : <div className="quality-empty large"><b>Select A Quality Item</b></div>}</main></div> : null}

    {view === "Punch & Closeout" ? <section className="punch-closeout"><header><div><p>SUBSTANTIAL COMPLETION · {formatDate(project.substantialDate)}</p><h2>Final Punch & Closeout Tracking</h2></div><i className={data?.closeout.reachedSubstantialCompletion ? "active" : "upcoming"}>{data?.closeout.reachedSubstantialCompletion ? "ACTIVE CLOSEOUT TRACKING" : "UPCOMING"}</i></header><div className="punch-columns">{["Open", "Awaiting Verification", "Acceptance", "Closed"].map((column) => <section key={column}><h3>{column}</h3>{data?.items.filter((item) => punchColumn(item.status) === column).map((item) => <button key={item.id} onClick={() => { setSelectedItemId(item.id); setDetailOpen(true); setView("Quality Items"); }}><b>{item.id}</b><span>{item.title}</span><small>{String(item.data.exactLocation || "")} · {String(item.data.responsibleTrade || "")}</small><i>{item.status}</i></button>)}{!data?.items.some((item) => punchColumn(item.status) === column) ? <div className="quality-empty mini">No Items</div> : null}</section>)}</div></section> : null}
