...
if command -v timeout >/dev/null 2>&1; then
  run_bounded() {
    local kill_after="$1" duration="$2"
    shift 2
    timeout --signal=TERM --kill-after="${kill_after}" "${duration}" "$@"
  }
else
  echo "GNU timeout is unavailable (install coreutils for it via Homebrew if you want one) — running build steps without a hang-safety timeout." >&2
  run_bounded() {
    shift 2
    "$@"
  }
...
run_bounded "${SITES_TYPECHECK_KILL_AFTER:-10s}" "${SITES_TYPECHECK_TIMEOUT:-4m}" \
  "${typescript_compiler}" --noEmit --pretty false

echo "Running static control verification..."
run_bounded "${SITES_LINT_KILL_AFTER:-10s}" "${SITES_LINT_TIMEOUT:-6m}" \
...
run_bounded "${SITES_BUILD_KILL_AFTER:-10s}" "${SITES_BUILD_TIMEOUT:-3m}" \
  "${vinext}" build

"${script_dir}/validate-artifact.sh"

echo "Running F-06 behavioral, runtime, role, migration, fault, and invariant controls..."
run_bounded "${SITES_TEST_KILL_AFTER:-10s}" "${SITES_TEST_TIMEOUT:-6m}" \
  node \
...
run_bounded "${SITES_MUTATION_KILL_AFTER:-10s}" "${SITES_MUTATION_TIMEOUT:-1m}" \
